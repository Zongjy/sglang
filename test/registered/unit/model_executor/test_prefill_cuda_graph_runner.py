...
class TestPrefillCudaGraphRunnerChunkedPrefix(CustomTestCase):
    ...
    def test_eagle_target_tc_piecewise_skips_last_mode_capture(self):
        ...
        with patch.object(
            ...
            side_effect=lambda _phase, backend: backend == Backend.TC_PIECEWISE,
        ):
            capture = capture_prefill_graph(
                model_runner=model_runner,
                eager_runner=eager_runner,
    ...
    def test_eagle_target_full_reaches_graph_construction(self):
        model_runner = SimpleNamespace(
            ...
            server_args=SimpleNamespace(
                ...
                enable_lora=False,
                cuda_graph_config=SimpleNamespace(
                    prefill=SimpleNamespace(bs=[1], backend=Backend.FULL)
                ),
            ),
        )
        ...
        with (
            patch.object(
                graph_setup,
                "check_cuda_graph_backend",
                side_effect=lambda _phase, backend: backend == Backend.FULL,
            ),
            patch.object(
                graph_setup,
