...
class TestPrefillCudaGraphRunnerHelpers(CustomTestCase):
    ...
    def test_prefill_buffers_preserve_dspark_aux_and_skip_first_rank_input(self):
        kwargs = dict(
            device=torch.device("cpu"),
            max_bs=4,
            max_num_tokens=16,
            ...
            pp_size=2,
            hc_hidden_size=12,
            pp_proxy_dspark_num_layers=3,
        )

        incoming = PrefillInputBuffers.create(**kwargs, is_first_pp_rank=False)
        self.assertEqual(
            {
                key: tuple(value.shape)
                for key, value in incoming.pp_proxy_tensors.items()
            },
            {"hidden_states": (16, 12), "dspark_aux": (16, 3, 8)},
        )

        first_rank = PrefillInputBuffers.create(**kwargs, is_first_pp_rank=True)
        self.assertIsNone(first_rank.pp_proxy_tensors)
